...
const tasksController = {
  listarTasks: async (_req, res) => {
    try {
      ...
      if (response.length === 0) {
        return res
          .status(404)
          .json({ message: "Nenhuma tarefa se encontra registrada." });
      }

      return res
        .status(200)
        .json({ message: "Tarefas Listadas:", result: response });
    } catch (error) {
      console.error(error);
      return res
        .status(500)
  ...
  atualizarTask: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Tarefa atualizada com sucesso", result: response });
    } catch (error) {
      console.error(error);
  ...
  deletarTask: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Tarefa deletada com sucesso", result: response });
    } catch (error) {
      console.error(error);
      return res.status(500).json({
        message: "Erro no servidor",
