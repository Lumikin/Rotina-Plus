...
// Monta a tela de cadastro com os campos usados na tabela clientes.
app.innerHTML = `
...
// Mostra mensagens de sucesso ou erro abaixo do formulario.
const showMessage = (text, type) => {
  message.textContent = text;
  message.className = `message ${type}`;
};

// Envia os dados do cadastro para a API do backend.
form.addEventListener("submit", async (event) => {
  event.preventDefault();

  // FormData permite ler os inputs pelo atributo name.
  ...
  // Usa a validacao nativa do navegador antes da requisicao.
  if (!form.checkValidity()) {
    form.reportValidity();
    return;
  }

  // Evita enviar o cadastro quando as senhas nao conferem.
  ...
  try {
    // Endpoint responsavel por inserir o cliente no banco de dados.
    ...
    // Exibe a mensagem retornada pela API quando o cadastro falha.
    if (!response.ok) {
