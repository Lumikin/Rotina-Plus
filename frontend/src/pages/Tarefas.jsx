...
export default function Tarefas() {
  ...
  async function handleCreate(event) {
    ...
    setCreating(true);
    setActionError("");
    const response = await criarTask({ userId, ...formData, status: "Pendente" });
    setCreating(false);
      ...
      <section className="tasks-shell">
        ...
        <div className="tasks-toolbar">
          <div className="tasks-tabs">
            <button type="button" className={activeView === "mine" ? "task-tab active" : "task-tab"} onClick={() => setActiveView("mine")}>
              Minhas tarefas
            </button>
            <button type="button" className={activeView === "all" ? "task-tab active" : "task-tab"} onClick={() => setActiveView("all")}>
              Todas as tarefas
            </button>
          </div>
                  ...
                  {isEditing ? (
                    <form className="task-edit-form" onSubmit={(e) => handleSaveEdit(e, task.UUID)}>
                      ...
                      <input
                        ...
                        required
                      />
                      <select className="task-form-input" value={editFormData.prioridade} onChange={(e) => onEditChange("prioridade", e.target.value)}>
                        {PRIORIDADES.map((opcao) => (
                          <option key={opcao} value={opcao}>{opcao}</option>
                        ))}
                      </select>
                      <select className="task-form-input" value={editFormData.status} onChange={(e) => onEditChange("status", e.target.value)}>
                        {STATUS.map((opcao) => (
                    ...
                    <>
                      <div className="task-main">
                        <div className="task-title-row">
                          <h2>{task.nome}</h2>
                          <span className={`status-badge ${getStatusClass(status)}`}>{STATUS_LABEL[status] ?? status}</span>
                        </div>
                      ...
                      <div className="task-actions">
                        <select value={status} disabled={isSaving} onChange={(e) => handleStatusChange(task.UUID, e.target.value, status)}>
                          {STATUS.map((opcao) => (
                            <option key={opcao} value={opcao}>{STATUS_LABEL[opcao]}</option>
                          ))}
                        </select>
                        <button type="button" className="edit-button" disabled={isSaving} onClick={() => handleEdit(task)}>Editar</button>
                        <button type="button" className="delete-button" disabled={isSaving} onClick={() => handleDelete(task.UUID)}>Excluir</button>
                      </div>
                    </>
                  )}
                </article>
